...
related: ["[[Сигнальное хранилище авторизации (Auth Store).md]]", "[[Функциональный перехватчик запросов (Auth Interceptor).md]]"]
status: "completed"
---
...
*   **Автоматическое обновление токенов (Silent Refresh)** — это бесшовная механика ротации безопасности JWT-токенов. Она перехватывает сетевые ошибки `401 Unauthorized` (свидетельствующие об истечении срока действия короткоживущего `accessToken`) и выполняет невидимый для пользователя запрос на получение новой пары токенов с помощью долгоживущего `refreshToken`.
*   **Функциональный перехватчик `HttpInterceptorFn`** используется в Angular для перехвата всех исходящих HTTP-запросов и ответов в одном месте.
*   **Предотвращение дублирующих запросов:** Главная сложность паттерна — блокировка параллельных запросов к API. Если 5 сетевых запросов одновременно завершатся ошибкой 401, интерцептор должен выполнить ровно один запрос на обновление токена, а остальные 4 запроса приостановить и запустить заново только после получения новой сессии.
*   **Используйте:** Во всех корпоративных веб-приложениях с авторизацией по протоколу OAuth2/JWT для обеспечения непрерывной сессии пользователя без принудительного логаута во время работы.
...
### Шаблон 1: Функциональный перехватчик с очередью ожидания токена (auth-refresh.interceptor.ts)
*   **Назначение:** Перехватчик автоматически подставляет JWT-токен во все запросы, отлавливает ошибки `401` и выстраивает очередь из параллельных запросов на время выполнения ротации токенов.
...
export const authRefreshInterceptor: HttpInterceptorFn = (req: HttpRequest<unknown>, next: HttpHandlerFn): Observable<HttpEvent<unknown>> => {
  ...
  // Считываем токен из сигнального хранилища
  ...
  return next(authReq).pipe(
    catchError((error: unknown) => {
      // Если сервер вернул 401 Unauthorized, запускаем процедуру Silent Refresh
...
### Шаблон 2: Сервис авторизации с методом обновления сессии (auth.service.ts)
...
export class AuthService {
  ...
  // Флаг критической секции для отслеживания активного процесса обновления
  ...
  public refreshToken(): Observable<AuthSession> {
    ...
    // Выполняем запрос на ротацию токенов
    return this.http.post<AuthSession>('https://api.web-archive.org/v1/auth/refresh', {
...
### Шаблон 3: Подключение перехватчика в конфигурации приложения (app.config.ts)
...
export const appConfig: ApplicationConfig = {
  providers: [
    ...
    // Регистрируем HttpClient и подключаем наш функциональный интерцептор
    provideHttpClient(
...
*   **`take(1)`** — критически важный оператор. Он заставляет подписку на `BehaviorSubject` автоматически закрыться (вызвать `complete`) сразу после того, как в нее придет первое валидное значение (новый токен). Без `take(1)` зависшие запросы останутся подписанными на `BehaviorSubject` навсегда, создавая тяжелую утечку памяти.
...
*   **Ошибка 1: Бесконечный цикл рекурсивных рефреш-запросов (Infinite 401 Loop)**
    ...
    *   *Решение:* Исключайте URL-адрес эндпоинта обновления токенов из обработки 401 ошибок внутри интерцептора.

```typescript
// ИСПРАВЛЕНИЕ внутри authRefreshInterceptor:
// Проверяем, что падающий запрос не является самим запросом на обновление токенов
