...
*   **Принцип цепочки обязанностей:** Перехватчик представляет собой чистую функцию, которая принимает исходящий запрос `HttpRequest` и ссылку на следующий обработчик в цепочке `HttpHandlerFn`, возвращая асинхронный поток событий `Observable<HttpEvent<unknown>>`.
*   **Иммутабельность сетевого слоя:** Объект `HttpRequest` строго защищен от прямой мутации. Любая модификация запроса (добавление заголовков, изменение URL, прикрепление параметров) должна выполняться исключительно посредством метода `.clone()`.
*   **Используйте для:** автоматической подстановки JWT-токенов авторизации, глобального перехвата и логирования сетевых ошибок, централизованного управления спиннерами загрузки или добавления системных заголовков (например, `X-Request-ID`).
...
*   **Назначение:** Перехватчик считывает токен доступа из локального хранилища, безопасно клонирует запрос и прикрепляет заголовок `Authorization: Bearer <token>` ко всем исходящим запросам к защищенному API.
...
*   **Назначение:** Глобальный обработчик сетевых сбоев перехватывает ошибки `401 Unauthorized` и `403 Forbidden`, перенаправляя пользователя на страницу авторизации, а также логирует системные сбои `5xx` во внешнюю систему мониторинга.
...
export class ProfileViewer implements OnInit { // Класс очищен от суффикса Component согласно стандартам
  ...
  public class: string = 'ProfileViewer';
...
Когда вы вызываете метод `next(req)` внутри перехватчика, вы не просто передаете запрос дальше по цепочке — вы возвращаете поток `Observable<HttpEvent<unknown>>`. Это дает интерцепторам уникальную возможность перехватывать и изменять данные ответа сервера на обратном пути движения потока (из сети к компоненту). Для этого используются стандартные RxJS-операторы (`tap`, `map`, `catchError`), висящие в трубе `pipe()` после вызова `next(req)`.
...
1.  **Повторные попытки отправки (Retry):**
    Если запрос завершился сетевым сбоем, и вы используете оператор `retry()`, Angular должен иметь возможность отправить исходный запрос заново. Если бы интерцепторы могли напрямую изменять свойства оригинального объекта запроса во время первого прохода, повторный запрос ушел бы в сеть в непредсказуемом, "загрязненном" состоянии.
