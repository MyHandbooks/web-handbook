...
*   **Назначение:** Описание чистого standalone-компонента, все зависимости которого инициализируются непосредственно при объявлении полей.
...
*   **Назначение:** Создание базового абстрактного класса с зависимостями и наследование дочернего компонента без необходимости писать конструктор и вызывать `super()`.
...
@Directive()
export abstract class BaseAnalyticsDirective {
...
@Component({
  ...
  standalone: true,
  template: `<button (click)="trackClick()">Отправить событие</button>`
})
export class CustomButtonComponent extends BaseAnalyticsDirective {
...
// Кастомная утилита для синхронизации заголовка вкладки с реактивным сигналом
...
@Component({
  ...
  standalone: true,
  template: `<h1>Динамическая страница</h1>`
})
...
Разберем выполнение вызова `useDocumentTitle(this.pageTitle)` в компоненте `DynamicPageComponent`:
1.  **Инстанцирование:** Angular начинает создавать `DynamicPageComponent`. Устанавливается активный контекст инжектора текущего компонента.
...
*   **Ошибка 2: Смешивание стилей внедрения (Гибридный подход)**
    ...
    *   *Решение:* Придерживайтесь одного выбранного стиля в рамках команды. Современный стандарт разработки в Angular 18/19 рекомендует полностью отказаться от конструкторов в пользу функционального внедрения через `inject()`.
